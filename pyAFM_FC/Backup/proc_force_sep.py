...
def proc_force_sep(name, k_c = 0.188, binsize = 1.0, dfit_win = 50, dfit_off = 60,
    ...
    clear = True, out=True, prefix = '', min_cont_pts=5, slope = 1.38):

    #read in a bunch of approach force curves
    #retract curves best handled in separate procedure
    ...
    if not os.path.exists(txtdir):
        os.makedirs(txtdir)
    txtdir = txtdir +"approach"
    outdir = txtdir+'_force_curves'+os.sep
    ...
    para_cols = ['index','slope', 'k_c', 'F_max', 'z_c' ]
    ...
    for fullname in namelist:
        ...
        zp, df, sep, zpb, deriv_df, zc = align_contact_reg(zpb, dfb, dfb_std, zp,
    ...
    plt.tight_layout()
    fig.savefig(outdir+'_df_deriv_bin'+ext)
    plt.close(fig)
    ...
    fig.savefig(outdir+'_cfit_hists'+ext)
    plt.close(fig)

    np.savetxt(txtdir+'_force_sep.txt', np.c_[sep,force,force_std])
    np.savetxt(outdir+'_deriv.txt', np.c_[zpb_bin,df_deriv_bin,df_deriv_std])

    f = open(txtdir+'parameter_log.txt','w')
    ...
    force_curves.to_csv(txtdir+'_force_curves.csv')
    curve_param.to_csv(txtdir+'_curve_param.csv')
...
def comp_def_deriv(name, k_c = 0.188, binsize = 1.0, dfit_win = 50, dfit_off = 60,
    ...
    clear = True, out=True, prefix = ''):

    #read in a bunch of approach force curves
    #retract curves best handled in separate procedure
    txtdir = name+"_force_curves"+os.sep+"approach"

    namelist = sorted(glob.glob(txtdir+os.sep+'*.txt'))
    ...
    plt.tight_layout()
    fig.savefig(txtdir+'_deriv_hist'+ext)
    plt.close(fig)
    ...
    #ax.set_ylim([-5,np.ceil(cfit_max)])
    plt.tight_layout()
    fig.savefig(txtdir+'_deriv_df_df_bin_zoom'+ext)
    plt.close(fig)


    deriv_curves.to_csv(txtdir+'_deriv_curves.csv')
...
def align_contact_reg(zpb, dfb, dfb_std, zp, df, w, dfit_win, cfit_max, out,
    ...
    zpcfit = zpb[w]
    dfcfit = dfb[w]
    # weights = 1.0/dfb_std[w]
    # pa = np.polyfit(zpcfit, dfcfit, 1, w=weights)
    ...
    return zp, df, sep, zpb, deriv_df, zc
